...
- an integer `vote` which indicates the chosen vote option
    + Generally, the following mapping SHOULD be used: `1` corresponds to "For", `2` corresponds to "Against", and `3` corresponds to "Abstain". Snapshot refers to this as the "Basic voting" voting type.
    + For a case with truly mutually exclusive options, an alternative mapping MAY be used, with the only limit being the maximum integer value. Snapshot refers to this as the "Single choice voting" voting type.
...
`results.power = .5 * √rpl`, where `rpl` is the amount of RPL staked in that node.
